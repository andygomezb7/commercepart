...
if (isset($_POST['guardar'])) {
    ...
    if (isset($_POST['id'])) {
        $id = intval($_POST['id']);
        // , descripcion = '$descripcion', precio = '$precio', marca_id = '$marca', categoria_id = '$categoria',codigo_original = '$originalcode'
        $db->query("UPDATE repuestos SET nombre = '$nombre' estado = '$estado'  WHERE id = $id");
        $mensaje .= 'El mensajero se ha actualizado correctamente.';
        ...
        $result = $db->query("INSERT INTO repuestos (nombre, fecha_creacion, imagen, estado,empresa_id) VALUES ('$nombre', '$fecha_creacion', '$imagen', '$estado', '".$_SESSION['empresa_id']."')");
        if ($result) {
            // INSERTAR LOS NUEVOS CODIGOS
            $id = $db->insert_id;
            // foreach ($codigos as $codigo) {
            ...
            //     $stmtInsertCodigo->execute();
            // }
            $mensaje .= 'El mensajero se ha agregado correctamente.';
        } else {
...
if (isset($_POST['eliminar'])) {
    ...
    $mensaje = 'El mensajero se ha eliminado correctamente.';
}
...
?>
        ...
        <?php if (isset($_GET['editar']) || isset($_GET['agregar'])) : ?>
            ...
            <h2><?php echo isset($_GET['editar']) ? 'Editar Mensajero' : 'Agregar Mensajero'; ?></h2>
