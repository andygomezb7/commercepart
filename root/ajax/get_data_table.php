...
switch ($method) {
    ...
    case 'compras':
        ...
        // Ejecutar la consulta y obtener los datos
        $start -= 1;
        $monedas = $db->query("SELECT c.*, bg.nombre AS surcursal, v.nombre AS vendedor FROM compras AS c LEFT JOIN bodegas AS bg ON c.bodega = bg.id LEFT JOIN usuarios AS v ON c.vendedor_id = v.id" . $search_ql . $order_ql . " LIMIT $start, $length");
