...
class Inventario {
    ...
    function insertarMovimientoInventario($repuestoId, $bodegaId, $tipo, $cantidad, $compraId = null, $pedidoId = null, $usuarioId = null, $comentario = null, $esReserva = false, $fechaEstimada = null) {
	    try {
	        ...
	        $query = "INSERT INTO $tabla (repuesto_id, bodega_id, tipo, cantidad, compra_id, pedido_id, usuario_id, comentario".($fechaEstimada&&$esReserva?",fecha_estimada":'').") 
	                  VALUES ('$repuestoId', '$bodegaId', '$tipo', '$cantidad', '$compraId', '$pedidoId', '$usuarioId', '$comentario'".($fechaEstimada&&$esReserva?",'$fechaEstimada'":'').")";
	        $stmt = $this->db->prepare($query);
	        $stmt->execute();
	...
	function insertarVentaDesdeReserva($repuestoId, $bodegaId, $cantidad, $usuarioId = null) {
	    try {
	        ...
	        $queryStockReserva = "SELECT cantidad FROM inventario_reserva WHERE repuesto_id = '$repuestoId' AND bodega_id = '$bodegaId'";
    ...
    try {
        ...
        $query .= "im.bodega_id, SUM(im.cantidad) AS total, ";
        ...
        if ($incluirReserva) {
            $query .= "(SELECT repuesto_id, bodega_id, cantidad, 'inventario' AS tipo FROM inventario_movimientos ";
            $query .= "UNION ALL ";
            $query .= "SELECT repuesto_id, bodega_id, cantidad, 'reserva' AS tipo FROM inventario_reserva) AS im ";
        } else {
